...
use std::str::FromStr;
use hedera::{Hbar, AccountId, TransferTransaction, TokenId, TransactionId, Result, Client};
...
pub fn create_transfer_transaction(token_id: String,
                                   ...
                                   token_amount: i64,
                                   decimals: u32,
                                    hedera_client: &Client) -> Result<Vec<u8>> {
    ...
    TransferTransaction::new()
        ...
        .token_transfer(token_id, owner_id, -(token_amount * ten.pow(decimals)))
        .token_transfer(token_id, payer_id, token_amount * ten.pow(decimals))
        .hbar_transfer(owner_id, Hbar::new(hbar_amount))
        .hbar_transfer(payer_id, Hbar::new(-hbar_amount))
        .freeze_with(hedera_client).unwrap().sign_with_operator(hedera_client).unwrap()
        .to_bytes()
}
